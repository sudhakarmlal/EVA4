...
def show_cam_on_image(img, mask,name):
    ...
    cam = cam / np.max(cam)
    print("Write Cam Image")
    image_name = "cam" + name+".jpg"
    cv2.imwrite(image_name, np.uint8(255 * cam))
...
def process_grad(images,model):
    ...
    #resnet = model
    #print(resnet)
    #print("After deleting Fc")
    del model.fc
    print(model)
    counter =1
    for img in images:
        print("Process Grad get gradcame")
        grad_cam = GradCam(model=models.vgg19(pretrained=True) , \
                        target_layer_names = ["35"], use_cuda=True)##这里改成layer4也很简单，我把每层name和size都打印出来了，想看哪层自
        ...
        grad_cam_images.append(show_cam_on_image(img, mask,str(counter)))
        counter = counter +1
